...
uint8_t get_threshold(const SDL_Surface *gray)
{
    ...
    uint64_t total_pixels = (size_t)gray->w * (size_t)gray->h;
    ...
    size_t wB = 0;   // Weight (count) of background
    size_t wF = 0;   // Weight (count) of foreground
    ...
    for (int t = 0; t < 256; t++)
    {
        ...
        sumB += (double)((size_t)t * histogram[t]);
